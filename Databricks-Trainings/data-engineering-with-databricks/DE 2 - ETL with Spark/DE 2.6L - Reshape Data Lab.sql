...
SELECT * FROM
(SELECT user_id as user, event_name, count(*) counta
FROM events
group by user_id, event_name) a
PIVOT ( 
  sum(counta) FOR event_name in("cart", "pillows", "login", "main", "careers", "guest", "faq", "down", "warranty", "finalize", 
...
-- THIS IS STUPID SOLUTION --> leaving for reference
...
-- MAGIC  .groupBy("user_id","event_name").agg(count("user_id").alias("counta"))
-- MAGIC  .groupBy("user_id")
-- MAGIC  .pivot("event_name")
-- MAGIC  .sum("counta")
-- MAGIC  .withColumnRenamed("user_id","user")
-- MAGIC  .createOrReplaceTempView("events_pivot_py"))
-- MAGIC
...
-- MAGIC %python
-- MAGIC check_table_results("events_pivot_py", 204586, ['user', 'cart', 'pillows', 'login', 'main', 'careers', 'guest', 'faq', 'down', 'warranty', 'finalize', 'register', 'shipping_info', 'checkout', 'mattresses', 'add_item', 'press', 'email_coupon', 'cc_info', 'foam', 'reviews', 'original', 'delivery', 'premium'])
...
-- MAGIC     .join(transactions_DF, events_pivot_DF.user == transactions_DF.user_id)
