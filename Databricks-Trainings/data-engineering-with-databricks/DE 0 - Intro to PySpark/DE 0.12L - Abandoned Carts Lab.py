...
converted_users_df = (sales_df.select(col("email")).distinct().withColumn("converted",lit(True))
                     )
...
# TODO
conversions_df = (users_df.join(converted_users_df,"email","outer").filter(col("email").isNotNull()).fillna(False,['converted'])
                 )
...
# TODO
abandoned_items_df = (abandoned_carts_df.select(explode(col("cart")).alias("items")).groupBy(col("items")).count()
                      #explode(col("cart").alias("item")).groupBy(col("item")).count()
                     )
