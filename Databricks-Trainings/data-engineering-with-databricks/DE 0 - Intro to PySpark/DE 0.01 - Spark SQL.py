...
products_df.select("name","price").collect()
...
products_df.select("name", "price").where("price < 200").orderBy("price")
...
budget_df.createOrReplaceTempView("events")
...
# MAGIC %sql
# MAGIC select * from marcin_jasinski_7z9y_da_delp.events
