...
timestamp_df = spark.table("events").select("user_id", col("event_timestamp").alias("timestamp"))
display(df)
...
data = [('1714533189000',), ('1714533194000',)]
...
epoch_time_seconds = 1714533194000 / 1000

# Convert to a datetime object
modification_time = datetime.datetime.utcfromtimestamp(epoch_time_seconds)
...
# Convert milliseconds since epoch to seconds since epoch
epoch_time_seconds = 1714533189000 / 1000
...
from pyspark.sql.functions import col
timestamp_df = df.withColumn("timestamp", (col("timestamp").cast("timestamp")))
