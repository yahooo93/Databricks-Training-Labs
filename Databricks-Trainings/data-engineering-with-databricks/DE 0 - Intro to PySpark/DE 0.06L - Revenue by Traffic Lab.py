...
traffic_df = df.groupBy(col("traffic_source")).agg(round(sum(df.revenue),1).alias("total_rev"), avg(df.revenue).alias("avg_rev"))
...
bonus_df = (top_traffic_df.withColumn("total_rev", round(col("total_rev"),2)).withColumn("avg_rev", round(col("avg_rev"),2))
...
chain_df = (df.groupBy(col("traffic_source"))
            .agg(round(sum(df.revenue),1)
                 .alias("total_rev"), avg(df.revenue)
                 .alias("avg_rev")).sort(col("total_rev").desc()).limit(3)
