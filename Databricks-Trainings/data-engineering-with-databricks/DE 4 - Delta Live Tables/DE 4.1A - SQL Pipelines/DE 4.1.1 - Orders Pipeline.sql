...
-- MAGIC %python
-- MAGIC display(dbutils.fs.ls('dbfs:/mnt/dbacademy-users/meet.marcin@gmail.com/data-engineer-learning-path/pipeline_demo/stream-source/orders'))
...
CREATE OR REFRESH STREAMING --LIVE
TABLE orders_silver
(CONSTRAINT valid_date EXPECT (order_timestamp > "2021-01-01") ON VIOLATION FAIL UPDATE) ---other option can be FAIL ROW - fails only this row, not entire FAIL UPDATE
...
CREATE OR REFRESH MATERIALIZED VIEW orders_by_date --CREATE OR REPLACE LIVE TABLE
AS 
SELECT 
